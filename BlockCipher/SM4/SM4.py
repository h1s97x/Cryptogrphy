import logging
from random import choice
from PyQt5 import QtCore
from Util import TypeConvert

xor = lambda a, b: list(map(lambda x, y: x ^ y, a, b))
rotl = lambda x, n: ((x << n) & 0xffffffff) | ((x >> (32 - n)) & 0xffffffff)
get_uint32_be = lambda key_data: ((key_data[0] << 24) | (key_data[1] << 16) | (key_data[2] << 8) | (key_data[3]))
put_uint32_be = lambda n: [((n >> 24) & 0xff), ((n >> 16) & 0xff), ((n >> 8) & 0xff), (n & 0xff)]
padding = lambda data, block=16: data + [(16 - len(data) % block) for _ in range(16 - len(data) % block)]
un_padding = lambda data: data[:-data[-1]]
list_to_bytes = lambda data: b''.join([bytes((i,)) for i in data])
bytes_to_list = lambda data: [i for i in data]
random_hex = lambda x: ''.join([choice('0123456789abcdef') for _ in range(x)])

# Expanded SM4 box table
SM4_BOXES_TABLE = [
    ,
]

# System parameter
SM4_FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc]

# fixed parameter
SM4_CK = [
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
]

SM4_ENCRYPT = 0
SM4_DECRYPT = 1


class Thread(QtCore.QThread):
    intermediate_value = QtCore.pyqtSignal(str)
    final_result = QtCore.pyqtSignal(str)
    logging_info = QtCore.pyqtSignal(str)

    def __init__(self, parent, input_text, key, mode):
        super(Thread, self).__init__(parent)
        self.input_text = input_text
        self.key = key
        self.mode = mode
        self.sk = [0] * 32
        self.set_key(self.key, mode)

    # Calculating round encryption key.
    # args:    [in] a: a is a 32 bits unsigned value;
    # return: sk[i]: i{0,1,2,3,...31}.
    @classmethod
    def _round_key(cls, ka):
        b = [0, 0, 0, 0]
        a = put_uint32_be(ka)
        b[0] = SM4_BOXES_TABLE[a[0]]
        b[1] = SM4_BOXES_TABLE[a[1]]
        b[2] = SM4_BOXES_TABLE[a[2]]
        b[3] = SM4_BOXES_TABLE[a[3]]
        bb = get_uint32_be(b[0:4])
        rk = bb ^ (rotl(bb, 13)) ^ (rotl(bb, 23))
        return rk

    # Calculating and getting encryption/decryption contents.
    # args:    [in] x0: original contents;
    # args:    [in] x1: original contents;
    # args:    [in] x2: original contents;
    # args:    [in] x3: original contents;
    # args:    [in] rk: encryption/decryption key;
    # return the contents of encryption/decryption contents.
    @classmethod
    def _f(cls, x0, x1, x2, x3, rk):
        # "T algorithm" == "L algorithm" + "t algorithm".
        # args:    [in] a: a is a 32 bits unsigned value;
        # return: c: c is calculated with line algorithm "L" and non-line algorithm "t"
        def _sm4_l_t(ka):
            b = [0, 0, 0, 0]
            a = put_uint32_be(ka)
            b[0] = SM4_BOXES_TABLE[a[0]]
            b[1] = SM4_BOXES_TABLE[a[1]]
            b[2] = SM4_BOXES_TABLE[a[2]]
            b[3] = SM4_BOXES_TABLE[a[3]]
            bb = get_uint32_be(b[0:4])
            c = bb ^ (rotl(bb, 2)) ^ (rotl(bb, 10)) ^ (rotl(bb, 18)) ^ (rotl(bb, 24))
            return c

        return x0 ^ _sm4_l_t(x1 ^ x2 ^ x3 ^ rk)

    # encrypt script
    def encrypt_run(self):
        logging.info("thread running")
        self.print_intermediate_value("/******************************Encryption begins*****************************/")
        self.print_intermediate_value("\nPlaintext:\n" + TypeConvert.hex_list_to_str(self.input_text))
        self.print_intermediate_value("Key:\n" + TypeConvert.hex_list_to_str(self.key))
        result = self.encrypt(self.input_text)
        self.print_intermediate_value("Encrypted:\n" + TypeConvert.hex_list_to_str(result))
        self.print_intermediate_value("Encryption completed\n")
        self.print_final_result(TypeConvert.hex_list_to_str(result))

    # decrypt script
    def decrypt_run(self):
        logging.info("thread running")
        self.print_intermediate_value("/******************************Encryption begins*****************************/")
        self.print_intermediate_value("\nCiphertext:\n" + TypeConvert.hex_list_to_str(self.input_text))
        self.print_intermediate_value("Key:\n" + TypeConvert.hex_list_to_str(self.key))
        result = self.decrypt(self.input_text)
        self.print_intermediate_value("Decrypted:\n" + TypeConvert.hex_list_to_str(result))
        self.print_intermediate_value("Decryption completed\n")
        self.print_final_result(TypeConvert.hex_list_to_str(result))
        # self._print_logging_info("Plaintext:  " + hexArr2HexStr(result))

    def print_intermediate_value(self, text):
        self.intermediate_value.emit(text)

    def print_final_result(self, text):
        self.final_result.emit(text)

    def _print_logging_info(self, text):
        self.logging_info.emit(text)

    # execute this function after start function executed
    def run(self):
        if self.mode == SM4_ENCRYPT:
            self.encrypt_run()
        else:
            self.decrypt_run()

    def set_key(self, key, mode):
        mk = [0, 0, 0, 0]
        k = [0] * 36

        mk[0] = get_uint32_be(key[0:4])
        mk[1] = get_uint32_be(key[4:8])
        mk[2] = get_uint32_be(key[8:12])
        mk[3] = get_uint32_be(key[12:16])
        k[0:4] = xor(mk[0:4], SM4_FK[0:4])
        for i in range(32):
            k[i + 4] = k[i] ^ (
                self._round_key(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ SM4_CK[i]))
            self.sk[i] = k[i + 4]
        self.mode = mode
        if mode == SM4_DECRYPT:
            for idx in range(16):
                t = self.sk[idx]
                self.sk[idx] = self.sk[31 - idx]
                self.sk[31 - idx] = t

    def one_round(self, sk, in_put):
        out_put = []
        ulbuf = [0] * 36
        ulbuf[0] = get_uint32_be(in_put[0:4])
        ulbuf[1] = get_uint32_be(in_put[4:8])
        ulbuf[2] = get_uint32_be(in_put[8:12])
        ulbuf[3] = get_uint32_be(in_put[12:16])
        for idx in range(32):
            ulbuf[idx + 4] = self._f(ulbuf[idx], ulbuf[idx + 1], ulbuf[idx + 2], ulbuf[idx + 3], sk[idx])
            # 显示RK[idx] 和 ulbuf[inx+4]
            self.print_intermediate_value(f'RK[{idx:02}]:  {sk[idx]:08X}      X[{idx:02}]:{ulbuf[idx + 4]:08X}')
        out_put += put_uint32_be(ulbuf[35])
        out_put += put_uint32_be(ulbuf[34])
        out_put += put_uint32_be(ulbuf[33])
        out_put += put_uint32_be(ulbuf[32])
        return out_put

    def encrypt(self, input_data):
        output_data = self.one_round(self.sk, input_data[0:16])
        return output_data

    def decrypt(self, input_data):
        output_data = self.one_round(self.sk, input_data[0:16])
        return output_data
