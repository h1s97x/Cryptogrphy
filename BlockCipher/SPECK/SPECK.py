...
class Thread(QtCore.QThread):
    ...
    def __init__(self, parent, input_text, key, encrypt_selected, key_size=128, block_size=128, encryption_mode='ECB',
                 init=0, counter=0):
        ...
        # Check Cipher Mode
        try:
            position = self.__valid_modes.index(self.encryption_mode)
    ...
    def decrypt_round(self, x, y, k):
        ...
        xor_xk = x ^ k
        m_sub = ((xor_xk - new_y) + self.mod_mask_sub) % self.mod_mask_sub
        new_x = ((m_sub >> (self.word_size - self.alpha_shift)) + (m_sub << self.alpha_shift)) & self.mod_mask
        return new_x, new_y
    ...
    def decrypt_function(self, upper_word, lower_word):
        ...
        for k in reversed(self.key_schedule):
            ...
            xor_xk = x ^ k
            m_sub = ((xor_xk - y) + self.mod_mask_sub) % self.mod_mask_sub
            x = ((m_sub >> (self.word_size - self.alpha_shift)) + (m_sub << self.alpha_shift)) & self.mod_mask
