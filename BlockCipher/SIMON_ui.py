...
class SIMONWidget(CryptographyWidget):
    def __init__(self):
        ...
        self.render()
        self.logging.log("SIMON algorithm has been imported.\n")
    ...
    def computer_encrypt(self):
        try:
            ...
            # initial SIMON thread
            thread = SIMON.Thread(self, plaintext, key, 0, self.KeyLen * 8, self.PlainLen * 8)
    ...
    def computer_decrypt(self):
        try:
            ...
            self.widgets_dict["Key"].set_text(TypeConvert.int_to_str(key, self.KeyLen))
            self.logging.log("Key:        " + TypeConvert.int_to_str(key, self.KeyLen))
            thread = SIMON.Thread(self, ciphertext, key, 1, self.KeyLen * 8, self.PlainLen * 8)
            # thread.intermediate_value.connect(self.widgets_dict["IntermediateValueTab"].append)
