...
class VigenereWidget(CryptographyWidget):
    def __init__(self):
        ...
        self.groups_config = [
            KeyGroup(
                name="Key",
                key_edit=[Key(id="Key", label="Key (Str)", default_text="best", enabled=True)],
                combo_box=[],
                buttons=[]
            ),
            Group(
                name="Encrypt",
                plain_text_edits=[
                    PlainTextEdit(id="Plaintext", label="Plaintext (Str)", default_text="data security"),
        ...
        self.render()
        self.log_message("Vigenere algorithm has been imported.\n")
