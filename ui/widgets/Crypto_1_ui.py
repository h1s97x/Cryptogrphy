...
from core.algorithms.symmetric.Crypto_1 import Thread as Crypto_1
...
class Crypto1Widget(CryptographyWidget):
    def __init__(self):
        ...
        self.render()
        self.log_message("Crypto-1 algorithm has been imported.\n")
    ...
    def computer_encrypt(self):
        try:
            ...
            if key_len != 6:
                self.log_message(ErrorType.LengthError.value + "Key length must be 48 bits.\n")
            ...
            if input_len != 4:
                self.log_message(ErrorType.LengthError.value + "Input length must be 32 bits.\n")
            ...
            if plaintext_len != 4:
                self.log_message(ErrorType.LengthError.value + "Plaintext length must be 32 bits.\n")
            ...
            input = TypeConvert.str_to_int(self.widgets_dict["Input"].get_text())
            self.log_message("Input:      " + TypeConvert.int_to_str(input, input_len))
    ...
    def computer_decrypt(self):
        try:
            ...
            if cipher_len != 4:
                self.log_message(ErrorType.LengthError.value + "Ciphertext length must be 32 bits.\n")
