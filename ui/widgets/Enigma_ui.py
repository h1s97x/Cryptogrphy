...
class EnigmaWidget(CryptographyWidget):
    def __init__(self):
        ...
        self.render()
        self.log_message("Enigma algorithm has been imported.\n")
    ...
    def computer_encrypt(self):
        try:
            ...
            if len(plugs) != len(set(plugs)):
                self.log_message(ErrorType.NotMeetRequirementError.value + "There are duplicate elements\n")
                self.pop_message_box(ErrorType.NotMeetRequirementError.value+ "There are duplicate elements")
                return
            if len(ring_setting) != 4:
                self.log_message(ErrorType.LengthError.value + "Ring setting length must be 4 bytes.\n")
                self.pop_message_box(ErrorType.LengthError.value + "Ring setting length must be 4 bytes.")
                return
            if len(start_position) != 4:
                self.log_message(ErrorType.LengthError.value + "Start position length must be 4 bytes.\n")
                self.pop_message_box(ErrorType.LengthError.value + "Start position length must be 4 bytes.")
                return
            if len(plugs) % 2 != 0:
                self.log_message(ErrorType.LengthError.value + "Plugs length must be even.\n")
            ...
            for i in range(len(ring_setting)):
                if not ring_setting[i].isupper():
                    self.log_message(ErrorType.CharacterError.value + " You should check the \"Ring Setting\" input box.\n")
            ...
            for i in range(len(start_position)):
                if not start_position[i].isupper():
                    self.log_message(ErrorType.CharacterError.value + " You should check the \"Start position\" input box.\n")
            ...
            for i in range(len(plugs)):
                if not plugs[i].isupper():
                    self.log_message(ErrorType.CharacterError.value + " You should check the \"Plugs\" input box.\n")
                    self.pop_message_box(ErrorType.CharacterError.value + " You should check the \"Plugs\" input box.")
                    return
            for i in range(len(plaintext)):
                if not plaintext[i].isupper():
                    self.log_message(ErrorType.CharacterError.value + " You should check the \"Plaintext\" input box.\n")
                    self.pop_message_box(ErrorType.CharacterError.value + " You should check the \"Plaintext\" input box.")
                    return
            # 明文不能为空
            if plaintext == '':
                self.log_message(ErrorType.NotMeetRequirementError.value + " You should check the \"Plaintext\" input box.\n")
            ...
            self.log_message("Ring Setting:   " + ring_setting)
            self.log_message("Start position: " + start_position)
            plugs_str = ''
            for i in range(0, len(plugs_temp)):
                plugs_str = plugs_str + plugs_temp[i] + " "
            self.log_message("Plugs:          " + plugs_str)
        ...
        try:
            ...
            for i in range(len(ciphertext)):
                if not ciphertext[i].isupper():
                    self.log_message(ErrorType.CharacterError.value + "You should check the \"Ciphertext\" input box.\n")
                    self.pop_message_box(ErrorType.CharacterError.value + "You should check the \"Ciphertext\" input box.")
                    return
            # 密文不能为空
            if ciphertext == '':
                self.log_message(ErrorType.NotMeetRequirementError.value + "You should check the \"Ciphertext\" input box.\n")
