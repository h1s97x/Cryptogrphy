...
from ui.main_window import Button, PlainTextEdit, Key, Group, ErrorType, KeyGroup
...
class AESWidget(CryptographyWidget):
    def __init__(self):
        ...
        self.groups_config = [
            KeyGroup(
                ...
                combo_box=[],
                buttons=[
                    Button(id="KeyGenerate", name="Generate Key", clicked_function=self.generate_key)
                ]
            ),
            Group(name="Encrypt",
        ...
        self.render()
        self.log_message("AES algorithm has been imported.\n")
    ...
    def computer_encrypt(self):
        try:
            ...
            if len(plaintext_list) != 16:
                ...
                self.log_message("\n")
                return
            key_list = TypeConvert.str_to_hex_list(self.widgets_dict["Key"].text())
            ...
            self.log_message("Plaintext:  " + TypeConvert.int_to_str(plaintext, 16))
            key = TypeConvert.str_to_int(self.widgets_dict["Key"].text())
            self.widgets_dict["Key"].setText(TypeConvert.int_to_str(key, 16))
            self.log_message("Key:        " + TypeConvert.int_to_str(key, 16))
            # initial AES thread
    ...
    def computer_decrypt(self):
        try:
            ...
            self.log_message("Ciphertext: " + TypeConvert.int_to_str(ciphertext, 16))
            key = TypeConvert.str_to_int(self.widgets_dict["Key"].text())
            self.widgets_dict["Key"].setText(TypeConvert.int_to_str(key, 16))
            self.log_message("Key:        " + TypeConvert.int_to_str(key, 16))
    ...
    def set_up_key(self, key):
        self.key = key
        private_key = key
        self.log_message("Generate key completes.")
        self.log_message("Key: {}".format(private_key.decode('utf-8')))
        self.widgets_dict["Key"].setText(private_key.decode('utf-8'))
