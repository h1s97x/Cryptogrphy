...
from . import common, core, pem, prime, randnum
...
class PublicKey(AbstractKey):
    ...
    def _load_pkcs1_der(cls, keyfile: bytes) -> 'PublicKey':
        ...
        from Menu.CryptographicAlgorithm.PublicKeyCryptography.RSA.mm_rsa.asn1 import AsnPubKey
    ...
    def load_pkcs1_openssl_der(cls, keyfile: bytes) -> 'PublicKey':
        ...
        from Menu.CryptographicAlgorithm.PublicKeyCryptography.RSA.mm_rsa.asn1 import OpenSSLPubKey
        from pyasn1.codec.der import decoder
...
def find_p_q(nbits: int, getprime_func=prime.getprime, accurate=True, shift_select=True) -> typing.Tuple[int, int]:
    ...
    >>> from Menu.CryptographicAlgorithm.PublicKeyCryptography.RSA.mm_rsa import common
    >>> (p, q) = find_p_q(128)
    >>> common.bit_size(p * q)
    256
    ...
    # http://www.di-mgt.com.au/mm_rsa_alg.html#crt
...
def gen_keys(nbits: int,
             ...
             accurate=True, shift_select=True,
    ...
    :param getprime_func: either :py:func:`mm_rsa.prime.getprime` or a function
    ...
    # ValueError.
    while True:
        (p, q) = find_p_q(nbits // 2, getprime_func, accurate, shift_select)
        try:
...
def newkeys(nbits: int, accurate=True, shift_select=True, poolsize=1, exponent=DEFAULT_EXPONENT) \
        -> typing.Tuple[PublicKey, PrivateKey]:
    """Generates public and private keys, and returns them as (pub, priv).

    The public key is also known as the 'encryption key', and is a
    :py:class:`mm_rsa.PublicKey` object. The private key is also known as the
    'decryption key' and is a :py:class:`mm_rsa.PrivateKey` object.
    ...
    :returns: a tuple (:py:class:`mm_rsa.PublicKey`, :py:class:`mm_rsa.PrivateKey`)
    ...
    if poolsize > 1:
        from Menu.CryptographicAlgorithm.PublicKeyCryptography.RSA.mm_rsa import parallel
    ...
    (p, q, e, d) = gen_keys(nbits, getprime_func, accurate=accurate, shift_select=shift_select, exponent=exponent)
